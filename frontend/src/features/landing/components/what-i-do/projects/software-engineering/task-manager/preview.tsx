        ...
        <div className="flex flex-col gap-4 p-4 border rounded-xl w-full max-w-md">
            <div className="flex gap-2">
                <input
                    ...
                    className="flex-1 px-3 py-2 border rounded-xl dark:bg-neutral-800 dark:border-neutral-700"
                />
                <button
                    onClick={addTask}
            ...
            <div className="space-y-2">
                {tasks.map(task => (
                    <div key={task.id} className="flex items-center justify-between p-3 border rounded-xl dark:border-neutral-700">
            ...
            {notifications.length > 0 && (
                <div className="mt-4 space-y-2">
                    {notifications.map((notification, index) => (
                        <div key={index} className="p-2 bg-green-100 dark:bg-green-900 dark:text-green-100 rounded-xl text-sm">
                            {notification}
                        </div>
                    ))}
                </div>
